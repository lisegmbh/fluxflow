...
`META-INF/fluxflow/type-manifest.properties` in a plain JAR. In an executable Spring Boot JAR, the
resource is stored below `BOOT-INF/classes`. `check` verifies the exact generated bytes in every
configured JAR, so a missing or stale manifest fails the build. Explicit types must be available
on the runtime classpath; a `compileOnly` declaration fails manifest generation.
